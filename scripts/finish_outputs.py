...
def main() -> None:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="配音：OpenAI 口语化配音稿（需 OPENAI_API_KEY）",
    )
    ap.add_argument(
        "--dub-tts-polish-openai",
        action="store_true",
        help="配音：OpenAI TTS 二次润色（需 OPENAI_API_KEY）",
    )
    ap.add_argument(
        "--dub-max-speedup",
        type=float,
    ...
    ap.add_argument(
        ...
        default="auto",
        help="配音：auto / edge / volc / elevenlabs",
    )
    ap.add_argument(
        "--volc-format",
        default="wav",
    ...
    if args.skip_dub and dub_audio.exists():
        ...
        if args.dub_colloquial_openai:
            dub_extra.append("--colloquial-openai")
        if args.dub_tts_polish_openai:
            dub_extra.append("--tts-polish-openai")
