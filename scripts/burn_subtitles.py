...
def burn(
    ...
    *,
    ...
    try:
        ...
        if mv is None:
            try:
                mv = int(os.getenv("BURN_SUBTITLE_MARGINV", "36").strip() or "36")
            except ValueError:
                mv = 36
        mv = max(8, min(mv, 120))
        vf = (
            f"subtitles='{sub_esc}':force_style="
            f"'Fontname={fn},Fontsize={fs},Outline={ol},Shadow=0,MarginV={mv},WrapStyle=0'"
        )
        cmd = [
            ffmpeg,
            "-y",
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="PX",
        help="底边距（默认 36，或 BURN_SUBTITLE_MARGINV）",
    )
    args = ap.parse_args()
    burn(
        args.video,
