...
def _split_en_two(en: str, ratio: float = 0.5) -> tuple[str, str]:
    ...
    target = max(2, min(len(en) - 2, int(len(en) * ratio)))
    sp = en.rfind(" ", 0, target + 1)
    if sp <= 0:
        sp = en.find(" ", target)
...
def _needs_split(en: str, zh: str, max_en: int, max_zh: int) -> bool:
    return len(en) > max_en or len(zh) > max_zh
...
def main() -> None:
    ...
    ap.add_argument(
        "--max-en-chars",
        type=int,
        default=int(os.getenv("BILINGUAL_MAX_EN_CHARS", "54")),
        help="单行英文超过该长度则拆条（可用环境变量 BILINGUAL_MAX_EN_CHARS）",
    )
    ap.add_argument(
        "--max-zh-chars",
        type=int,
        default=int(os.getenv("BILINGUAL_MAX_ZH_CHARS", "28")),
    ...
    merge(
        ...
        args.zh,
        outp,
        max_en_chars=max(24, args.max_en_chars),
        max_zh_chars=max(12, args.max_zh_chars),
    )
