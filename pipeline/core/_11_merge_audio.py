...
def _parse_cell(value):
    """Parse list cells from Excel without eval(np.*) NameError."""
    if not isinstance(value, str):
        return value
    cleaned = re.sub(r"np\.float\d+\(([^)]+)\)", r"\1", value)
...
def load_and_flatten_data(excel_file):
    ...
    lines = [_parse_cell(line) for line in df['lines'].tolist()]
    lines = [item for sublist in lines for item in sublist]

    new_sub_times = [_parse_cell(time) for time in df['new_sub_times'].tolist()]
...
def get_audio_files(df):
    ...
    for index, row in df.iterrows():
        number = row['number']
        line_count = len(_parse_cell(row['lines']))
