#!/usr/bin/env python3
"""
【模块】elevenlabs_tts_client.py — ElevenLabs HTTP TTS（eleven_multilingual_v2），支持 stability / similarity_boost。
【调用方】dub_zh.py（--backend elevenlabs）。
...
def synthesize_elevenlabs_tts(
    ...
    *,
    ...
    st = 0.5 if stability is None else max(0.0, min(1.0, float(stability)))
    sim = 0.75 if similarity_boost is None else max(0.0, min(1.0, float(similarity_boost)))
    body = json.dumps(
        {
            "text": text,
            "model_id": model_id,
            "voice_settings": {"stability": st, "similarity_boost": sim},
        },
        ensure_ascii=False,
    ).encode("utf-8")
